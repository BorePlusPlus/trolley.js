(function(window, document) {
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4) {
                if(xhr.status === 200) {
                    ...
                    fileObj.immediate ? insertFile(storedObj) : loadViaQ(qi, storedObj);
                } else {
    ...
    function lazyLoad(fileObj) {
        ...
        if (!fileObj.immediate) loadQ[qi] = { loaded: false };
        if (hasLocalStorage) {
            storedObj = JSON.parse(localStorage[fileObj.url] || 'false');
            if (storedObj) return fileObj.immediate ? insertFile(storedObj) : loadViaQ(qi, storedObj);
        }
        load(fileObj, qi);
    }

    function loadViaQ(qi, storedObj) {
        var loadObj;

        loadQ[qi].storedObj = storedObj;
        for (var i = 0, length = loadQ.length; i < length; i++) {
            loadObj = loadQ[i];
            if (!loadObj.loaded) {
