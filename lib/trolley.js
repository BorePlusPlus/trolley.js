(function(window, document) {
    ...
    function eachTrolliedItem(callback) {
        ...
        for (item in localStorage) {
            if (item.indexOf(storePrefix) === 0) {
                ...
                storedObj = JSON.parse(localStorage[item]);
                callback(item, storedObj);
            }
        }
    }
    ...
    function checkVersionUpdate(version) {
        ...
        currentVersionNumber = JSON.parse(localStorage[versionsKey] || '{}')[namespace]; // duplicate
    ...
    function updateVersion(version) {
        ...
        currentVersions = JSON.parse(localStorage[versionsKey] || '{}'); // duplicate
